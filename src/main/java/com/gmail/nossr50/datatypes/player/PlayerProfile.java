...
public class PlayerProfile {
  ...
  private final Map<PrimarySkillType, Integer> skills =
      new HashMap<PrimarySkillType, Integer>(); // Skill & Level
  private final Map<PrimarySkillType, Float> skillsXp =
      new HashMap<PrimarySkillType, Float>(); // Skill & XP
  private final Map<SuperAbilityType, Integer> abilityDATS =
      new HashMap<SuperAbilityType, Integer>(); // Ability & Cooldown
  private final Map<UniqueDataType, Integer> uniquePlayerData =
      new HashMap<>(); // Misc data that doesn't fit into other categories
                       // (chimaera wing, etc..)

  // Store previous XP gains for diminished returns
  private DelayQueue<SkillXpGain> gainedSkillsXp =
      new DelayQueue<SkillXpGain>();
  private HashMap<PrimarySkillType, Float> rollingSkillsXp =
      new HashMap<PrimarySkillType, Float>();
  ...
  public PlayerProfile(String playerName, UUID uuid) {
    ...
    for (PrimarySkillType primarySkillType :
         PrimarySkillType.NON_CHILD_SKILLS) {
      skills.put(primarySkillType,
                 AdvancedConfig.getInstance().getStartingLevel());
  ...
  public PlayerProfile(String playerName, UUID uuid,
                       Map<PrimarySkillType, Integer> levelData,
                       Map<PrimarySkillType, Float> xpData,
                       Map<SuperAbilityType, Integer> cooldownData,
                       MobHealthbarType mobHealthbarType,
                       int scoreboardTipsShown,
                       Map<UniqueDataType, Integer> uniqueProfileData) {
    this.playerName = playerName;
    this.uuid = uuid;
  ...
  public void scheduleAsyncSaveDelay() {
    new PlayerProfileSaveTask(this, false)
        .runTaskLaterAsynchronously(mcMMO.p, 20);
  }
  ...
  public void save(boolean useSync) {
    ...
    PlayerProfile profileCopy = new PlayerProfile(
        playerName, uuid, ImmutableMap.copyOf(skills),
        ImmutableMap.copyOf(skillsXp), ImmutableMap.copyOf(abilityDATS),
        mobHealthbarType, scoreboardTipsShown,
        ImmutableMap.copyOf(uniquePlayerData));
    changed = !mcMMO.getDatabaseManager().saveUser(profileCopy);

    if (changed) {
      mcMMO.p.getLogger().severe(
          "PlayerProfile saving failed for player: " + playerName + " " + uuid);

      if (saveAttempts > 0) {
        mcMMO.p.getLogger().severe("Attempted to save profile for player " +
                                   getPlayerName() + " resulted in failure. " +
                                   saveAttempts + " have been made so far.");
      }
      ...
      if (saveAttempts < 10) {
        ...
        mcMMO.p.getLogger().severe(
            "mcMMO has failed to save the profile for " + getPlayerName() +
            " numerous times."
  ...
  public MobHealthbarType getMobHealthbarType() { return mobHealthbarType; }
   ...
   * save operation
   */
  public void markProfileDirty() { changed = true; }

  public int getScoreboardTipsShown() { return scoreboardTipsShown; }
  ...
  public void registerXpGain(PrimarySkillType primarySkillType, float xp) {
    ...
    rollingSkillsXp.put(primarySkillType,
                        getRegisteredXpGain(primarySkillType) + xp);
  }
