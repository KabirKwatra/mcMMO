...
public class McMMOPlayer {
  ...
  private final Map<PrimarySkillType, SkillManager> skillManagers =
      new HashMap<PrimarySkillType, SkillManager>();
  ...
  private final Map<SuperAbilityType, Boolean> abilityMode =
      new HashMap<SuperAbilityType, Boolean>();
  private final Map<SuperAbilityType, Boolean> abilityInformed =
      new HashMap<SuperAbilityType, Boolean>();

  private final Map<ToolType, Boolean> toolMode =
      new HashMap<ToolType, Boolean>();
  ...
  public McMMOPlayer(Player player, PlayerProfile profile) {
    ...
    /*
     * I'm using this method because it makes code shorter and safer (we don't
     * have to add all SkillTypes manually), but I actually have no idea about
     * the performance impact, if there is any. If in the future someone wants
     * to remove this, don't forget to also remove what is in the
     * PrimarySkillType enum. - bm01
     */
    try {
      for (PrimarySkillType primarySkillType : PrimarySkillType.values()) {
        skillManagers.put(primarySkillType,
                          primarySkillType.getManagerClass()
                              .getConstructor(McMMOPlayer.class)
                              .newInstance(this));
      }
    } catch (Exception e) {
      e.printStackTrace();
    ...
    debugMode = false; // Debug mode helps solve support issues, players can
                       // toggle it on or off
    attackStrength = 1.0D;
  }
  ...
  public void processPostXpEvent(PrimarySkillType primarySkillType,
                                 Plugin plugin, XPGainSource xpGainSource) {
    // Check if they've reached the power level cap just now
    if (hasReachedPowerLevelCap()) {
      NotificationManager.sendPlayerInformationChatOnly(
          player, "LevelCap.PowerLevel",
          String.valueOf(Config.getInstance().getPowerLevelCap()));
    } else if (hasReachedLevelCap(primarySkillType)) {
      NotificationManager.sendPlayerInformationChatOnly(
          player, "LevelCap.Skill",
          String.valueOf(Config.getInstance().getLevelCap(primarySkillType)),
          primarySkillType.getName());
    }

    // Updates from Party sources
    if (xpGainSource == XPGainSource.PARTY_MEMBERS &&
        !ExperienceConfig.getInstance().isPartyExperienceBarsEnabled())
      return;

    // Updates from passive sources (Alchemy, Smelting, etc...)
    if (xpGainSource == XPGainSource.PASSIVE &&
        !ExperienceConfig.getInstance().isPassiveGainsExperienceBarsEnabled())
      return;

    updateXPBar(primarySkillType, plugin);
  }

  public void processUnlockNotifications(mcMMO plugin,
                                         PrimarySkillType primarySkillType,
                                         int skillLevel) {
    RankUtils.executeSkillUnlockNotifications(plugin, this, primarySkillType,
                                              skillLevel);
  }
  ...
  public double
  getProgressInCurrentSkillLevel(PrimarySkillType primarySkillType) {
  ...
  public boolean getAbilityUse() { return abilityUse; }

  public void toggleAbilityUse() { abilityUse = !abilityUse; }
  ...
  public int getRecentlyHurt() { return recentlyHurt; }

  public void setRecentlyHurt(int value) { recentlyHurt = value; }
  ...
  public int getChimeraWingLastUse() { return profile.getChimaerWingDATS(); }
  ...
  public Location getTeleportCommenceLocation() { return teleportCommence; }
  ...
  public int getRespawnATS() { return respawnATS; }
  ...
  public int getTeleportATS() { return teleportATS; }
  ...
  public long getDatabaseATS() { return databaseATS; }
  ...
  public boolean useChatNotifications() { return displaySkillNotifications; }
   ...
   * If they are at the power level cap, this will return true, otherwise it
   * checks their skill level
  ...
  public boolean hasReachedLevelCap(PrimarySkillType primarySkillType) {
    ...
    return getSkillLevel(primarySkillType) >=
        Config.getInstance().getLevelCap(primarySkillType);
  }
  ...
  /**
   * Begins an experience gain. The amount will be affected by skill modifiers,
   * global rate, perks, and may be shared with the party
   *
   * @param skill Skill being used
   * @param xp Experience amount to process
   */
  public void beginXpGain(PrimarySkillType skill, float xp,
  ...
  /**
   * Begins an experience gain. The amount will be affected by skill modifiers,
   * global rate and perks
   *
   * @param skill Skill being used
   * @param xp Experience amount to process
   */
  public void beginUnsharedXpGain(PrimarySkillType skill, float xp,
                                  XPGainReason xpGainReason,
                                  XPGainSource xpGainSource) {
   ...
   */
  public void applyXpGain(PrimarySkillType primarySkillType, float xp,
                          XPGainReason xpGainReason,
                          XPGainSource xpGainSource) {
    ...
    if (primarySkillType.isChildSkill()) {
      Set<PrimarySkillType> parentSkills =
          FamilyTree.getParents(primarySkillType);
    ...
    if (!EventUtils.handleXpGainEvent(player, primarySkillType, xp,
                                      xpGainReason)) {
      return;
    }
   ...
   */
  private void checkXp(PrimarySkillType primarySkillType,
    ...
    while (getSkillXpLevelRaw(primarySkillType) >=
           getXpToLevel(primarySkillType)) {
      if (hasReachedLevelCap(primarySkillType)) {
    ...
    if (EventUtils.tryLevelChangeEvent(player, primarySkillType, levelsGained,
    ...
    NotificationManager.sendPlayerLevelUpNotification(
        this, primarySkillType, levelsGained,
  ...
  public void setPartyInvite(Party invite) { this.invite = invite; }

  public Party getPartyInvite() { return invite; }

  public boolean hasPartyInvite() { return (invite != null); }

  public void setParty(Party party) { this.party = party; }

  public Party getParty() { return party; }

  public boolean inParty() { return (party != null); }

  public void removeParty() { party = null; }

  public void removePartyInvite() { invite = null; }

  public PartyTeleportRecord getPartyTeleportRecord() { return ptpRecord; }

  public void setPartyAllianceInvite(Party allianceInvite) {
    this.allianceInvite = allianceInvite;
  }

  public Party getPartyAllianceInvite() { return allianceInvite; }

  public boolean hasPartyAllianceInvite() { return (allianceInvite != null); }

  public void removePartyAllianceInvite() { allianceInvite = null; }

  public void loginParty() { party.addOnlineMember(this.getPlayer()); }
  ...
  public boolean isUsingUnarmed() { return isUsingUnarmed; }
  ...
  public void checkAbilityActivation(PrimarySkillType skill) {
    ...
    // TODO: This is hacky and temporary solution until skills are move to the
    // new system Potential problems with this include skills with two super
    // abilities (ie mining)
    if (!skill.isSuperAbilityUnlocked(getPlayer())) {
      int diff =
          RankUtils.getSuperAbilityUnlockRequirement(skill.getAbility()) -
          getSkillLevel(skill);

      // Inform the player they are not yet skilled enough
      NotificationManager.sendPlayerInformation(
          player, NotificationType.ABILITY_COOLDOWN,
    ...
    if (timeRemaining > 0) {
       ...
       */
      if (skill == PrimarySkillType.WOODCUTTING ||
          skill == PrimarySkillType.AXES) {
        NotificationManager.sendPlayerInformation(
            player, NotificationType.ABILITY_COOLDOWN, "Skills.TooTired",
    ...
    // These values change depending on whether or not the server is in retro
    // mode
    ...
    if (abilityLengthCap > 0) {
      ticks = PerksUtils.handleActivationPerks(
          player,
          2 + (Math.min(abilityLengthCap, getSkillLevel(skill)) /
               abilityLengthVar),
          ability.getMaxLength());
    } else {
      ticks = PerksUtils.handleActivationPerks(
          player, 2 + (getSkillLevel(skill) / abilityLengthVar),
          ability.getMaxLength());
    }
    ...
    if (useChatNotifications()) {
      NotificationManager.sendPlayerInformation(
          player, NotificationType.SUPER_ABILITY, ability.getAbilityOn());
      // player.sendMessage(ability.getAbilityOn());
    }

    SkillUtils.sendSkillMessage(player,
                                NotificationType.SUPER_ABILITY_ALERT_OTHERS,
                                ability.getAbilityPlayer());
  ...
  public void processAbilityActivation(PrimarySkillType skill) {
     ...
     * Basically the tool always needs to ready and we check to see if the
     * cooldown is over when the user takes action
     */
    if (tool.inHand(inHand) && !getToolPreparationMode(tool)) {
      ...
      if (Config.getInstance().getAbilityMessagesEnabled()) {
        NotificationManager.sendPlayerInformation(player, NotificationType.TOOL,
                                                  tool.getRaiseTool());
        SoundManager.sendSound(player, player.getLocation(),
                               SoundType.TOOL_READY);
      }

      setToolPreparationMode(tool, true);
      new ToolLowerTask(this, tool)
          .runTaskLater(mcMMO.p, 4 * Misc.TICK_CONVERSION_FACTOR);
    }
  }
  ...
  public int calculateTimeRemaining(SuperAbilityType ability) {
    long deactivatedTimestamp =
        profile.getAbilityDATS(ability) * Misc.TIME_CONVERSION_FACTOR;
    return (int)(((deactivatedTimestamp + (PerksUtils.handleCooldownPerks(
                                               player, ability.getCooldown()) *
  ...
  /*
   * These functions are wrapped from PlayerProfile so that we don't always have
   * to store it alongside the McMMOPlayer object.
   */
  ...
  public void resetCooldowns() { profile.resetCooldowns(); }

  public FixedMetadataValue getPlayerMetadata() { return playerMetadata; }
