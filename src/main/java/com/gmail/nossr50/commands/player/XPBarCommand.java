...
public class XPBarCommand implements TabExecutor {
    ...
    if (sender instanceof Player) {
      ...
      if (mmoPlayer == null) {
        NotificationManager.sendPlayerInformationChatOnlyPrefixed(
            mmoPlayer.getPlayer(), "Profile.PendingLoad");
        return false;
      }
      ...
      if (args.length == 0) {
        ...
        if (option.equalsIgnoreCase(
          ...
          mmoPlayer.getExperienceBarManager().xpBarSettingToggle(
              ExperienceBarManager.XPBarSettingTarget.RESET, null);
          return true;
        } else {
          return false;
        }
        ...
        if (SkillUtils.isSkill(skillName)) {
          ...
          ExperienceBarManager.XPBarSettingTarget settingTarget =
              getSettingTarget(option);
          if (settingTarget != null &&
              settingTarget != ExperienceBarManager.XPBarSettingTarget.RESET) {
            // Change setting
            mmoPlayer.getExperienceBarManager().xpBarSettingToggle(
                settingTarget, targetSkill);
            return true;
          } else {
            return false;
          }
  ...
  private @Nullable ExperienceBarManager.XPBarSettingTarget
  getSettingTarget(String string) {
  ...
  public List<String> onTabComplete(@NotNull CommandSender sender,
    ...
    case 1:
      ...
      for (ExperienceBarManager.XPBarSettingTarget settingTarget :
           ExperienceBarManager.XPBarSettingTarget.values()) {
      ...
      return StringUtil.copyPartialMatches(
          args[0], options,
          new ArrayList<>(
              ExperienceBarManager.XPBarSettingTarget.values().length));
    case 2:
