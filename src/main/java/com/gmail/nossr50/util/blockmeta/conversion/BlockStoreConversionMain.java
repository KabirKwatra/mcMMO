...
public class BlockStoreConversionMain implements Runnable {
  ...
  public BlockStoreConversionMain(org.bukkit.World world) {
    ...
    this.converters =
        new BlockStoreConversionXDirectory[HiddenConfig.getInstance()
                                               .getConversionRate()];
  }
  ...
  public void run() {
    ...
    for (this.i = 0;
         (this.i < HiddenConfig.getInstance().getConversionRate()) &&
         (this.i < this.xDirs.length);
         this.i++) {
  ...
  public void softStop() {
    ...
    mcMMO.p.getLogger().info("Finished converting the storage for " +
                             world.getName() + ".");
