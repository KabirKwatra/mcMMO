...
public class HashChunkletManager implements ChunkletManager {
  public HashMap<String, ChunkletStore> store =
      new HashMap<String, ChunkletStore>();
  ...
  public void unloadChunklet(int cx, int cy, int cz, World world) {
    ...
    if (store.containsKey(world.getName() + "," + cx + "," + cz + "," + cy)) {
      ...
      ChunkletStore out =
          store.get(world.getName() + "," + cx + "," + cz + "," + cy);
  ...
  public void unloadChunk(int cx, int cz, World world) {
    ...
    for (int y = 0; y < 4; y++) {
      if (store.containsKey(world.getName() + "," + cx + "," + cz + "," + y)) {
        ...
        ChunkletStore out =
            store.get(world.getName() + "," + cx + "," + cz + "," + y);
        serializeChunkletStore(out, yFile);
  ...
  public void setFalse(int x, int y, int z, World world) {
    ...
    if (cStore == null) {
      return; // No need to make a store for something we will be setting to
              // false
    }
  ...
  public void cleanUp() {
    for (String key : store.keySet()) {
      if (store.get(key).isEmpty()) {
        String[] info = key.split(",");
        File dataDir =
            new File(mcMMO.p.getServer().getWorld(info[0]).getWorldFolder(),
                     "mcmmo_data");
  ...
  private ChunkletStore deserializeChunkletStore(File location) {
    ...
    try {
      ...
      if (ex instanceof EOFException) {
        ...
        // mcMMO.p.getLogger().severe("Chunklet data at " + location.toString()
        // + " could not be read due to an EOFException, data in this area will
        // be lost.");
        ...
        // mcMMO.p.getLogger().severe("Chunklet data at " + location.toString()
        // + " is corrupted, data in this area will be lost.");
        return ChunkletStoreFactory.getChunkletStore();
      } else if (ex instanceof UTFDataFormatException) {
        // UTF happens when the Chunklet cannot be read or is corrupted
        // mcMMO.p.getLogger().severe("Chunklet data at " + location.toString()
        // + " could not be read due to an UTFDataFormatException, data in this
        // area will be lost.");
        return ChunkletStoreFactory.getChunkletStore();
      }
    ...
    // TODO: Make this less messy, as it is, it's kinda... depressing to do it
    // like this. Might also make a mess when we move to stacks, but at that
    // point I think I will write a new Manager... IMPORTANT! If
    // ChunkletStoreFactory is going to be returning something other than
    // PrimitiveEx we need to remove this, as it will be breaking time for old
    // maps
