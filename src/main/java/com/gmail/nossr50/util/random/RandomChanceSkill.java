...
public class RandomChanceSkill implements RandomChanceExecution {
  ...
  public RandomChanceSkill(Player player, SubSkillType subSkillType,
    ...
    if (hasCap)
      this.probabilityCap =
          AdvancedConfig.getInstance().getMaximumProbability(subSkillType);
    else
      this.probabilityCap = RandomChanceUtil.LINEAR_CURVE_VAR;
   ...
   */
  public SubSkillType getSubSkill() { return subSkillType; }
   ...
   */
  public void setSkillLevel(int newSkillLevel) { skillLevel = newSkillLevel; }

  /**
   * The maximum bonus level for this skill
   * This is when the skills level no longer increases the odds of success
   * For example, a value of 25 will mean the success chance no longer grows
   * after skill level 25
   *
   ...
   * For example, if this value is 10, then 10% odds would be the maximum and
   * would be achieved only when xPos equaled the LinearCurvePeak
   *
   * @return maximum probability odds from 0.00 (no chance of ever happened) to
   *     100.0 (probability can be guaranteed)
   */
  @Override
  ...
  public boolean isLucky() { return isLucky; }

  public double getResultModifier() { return resultModifier; }
