...
public final class ParticleEffectUtils {
  ...
  public static void playBleedEffect(LivingEntity livingEntity) {
    ...
    livingEntity.getWorld().playEffect(livingEntity.getEyeLocation(),
                                       Effect.STEP_SOUND,
  ...
  public static void playGreaterImpactEffect(LivingEntity livingEntity) {
    ...
    livingEntity.getWorld().createExplosion(location.getX(), location.getY(),
  ...
  /*    public static void fireworkParticleShower(Player player, Color color) {
          ...
          location.setY(location.getY() + (player.isInsideVehicle() ? 1.0 :
     -1.0)); location.setPitch(-90);

          Firework firework = (Firework) player.getWorld().spawnEntity(location,
     EntityType.FIREWORK); firework.setMetadata(mcMMO.funfettiMetadataKey, new
     FixedMetadataValue(mcMMO.p, null)); FireworkMeta fireworkMeta =
     firework.getFireworkMeta(); FireworkEffect effect =
     FireworkEffect.builder().flicker(false).withColor(color).with((Config.getInstance().getLargeFireworks()
     ? Type.BALL_LARGE : Type.BALL)).trail(true).build();
