...
public final class CombatUtils {
  ...
  private static void processSwordCombat(LivingEntity target, Player player,
    ...
    if (swordsManager.canUseStab()) {
      finalDamage +=
          (swordsManager.getStabDamage() * mcMMOPlayer.getAttackStrength());
    }
    ...
    if (canUseLimitBreak(player, target,
                         SubSkillType.SWORDS_SWORDS_LIMIT_BREAK)) {
      finalDamage +=
          (getLimitBreakDamage(player, target,
                               SubSkillType.SWORDS_SWORDS_LIMIT_BREAK) *
  ...
  //        BukkitPlatform bukkitPlatform = (BukkitPlatform)
  //        mcMMO.getPlatformManager().getPlatform();
  //        Bukkit.broadcastMessage("Strength:
  //        "+bukkitPlatform.getPlayerAttackStrength(player));
  //
  //        Bukkit.getScheduler().scheduleSyncDelayedTask(mcMMO.p, () -> {
  //            Bukkit.broadcastMessage("1 Tick Delay: " +
  //            bukkitPlatform.getPlayerAttackStrength(player));
  //        }, 1);
  //
  //        Bukkit.getScheduler().scheduleSyncDelayedTask(mcMMO.p, () -> {
  //            Bukkit.broadcastMessage("5 Tick Delay: " +
  //            bukkitPlatform.getPlayerAttackStrength(player));
  //        }, 5);
  //
  //        Bukkit.getScheduler().scheduleSyncDelayedTask(mcMMO.p, () -> {
  //            Bukkit.broadcastMessage("80 Tick Delay: " +
  //            bukkitPlatform.getPlayerAttackStrength(player));
  //        }, 20 * 4);
  //
  ...
  private static void processAxeCombat(LivingEntity target, Player player,
    ...
    if (canUseLimitBreak(player, target, SubSkillType.AXES_AXES_LIMIT_BREAK)) {
      finalDamage += (getLimitBreakDamage(player, target,
  ...
  private static void processUnarmedCombat(LivingEntity target, Player player,
                                           EntityDamageByEntityEvent event) {
    if (event.getCause() == DamageCause.THORNS) {
      return;
    }
    ...
    if (unarmedManager.canUseIronArm()) {
      finalDamage += (unarmedManager.calculateIronArmDamage() *
                      mcMMOPlayer.getAttackStrength());
    }
  ...
  private static void processArcheryCombat(LivingEntity target, Player player,
                                           EntityDamageByEntityEvent event,
                                           Projectile arrow) {
    double initialDamage = event.getDamage();
    ...
    if (target instanceof Player && PrimarySkillType.UNARMED.getPVPEnabled()) {
      UnarmedManager unarmedManager =
          UserManager.getPlayer((Player)target).getUnarmedManager();
    ...
    if (archeryManager.canDaze(target)) {
      finalDamage += archeryManager.daze(
    ...
    if (canUseLimitBreak(player, target,
      ...
      finalDamage += getLimitBreakDamage(
          player, target, SubSkillType.ARCHERY_ARCHERY_LIMIT_BREAK);
    }

    double distanceMultiplier =
        archeryManager.distanceXpBonusMultiplier(target, arrow);
    double forceMultiplier =
        1.0; // Hacky Fix - some plugins spawn arrows and assign them to players
             // after the ProjectileLaunchEvent fires

    if (arrow.hasMetadata(mcMMO.bowForceKey))
      forceMultiplier = arrow.getMetadata(mcMMO.bowForceKey).get(0).asDouble();

    applyScaledModifiers(initialDamage, finalDamage, event);
    startGainXp(mcMMOPlayer, target, PrimarySkillType.ARCHERY,
  ...
  public static void processCombatAttack(EntityDamageByEntityEvent event,
    ...
    if (target instanceof Player) {
      ...
      if (acrobaticsManager.canDodge(target)) {
        event.setDamage(
            acrobaticsManager.dodgeCheck(painSourceRoot, event.getDamage()));
      }
      ...
      if (ItemUtils.isSword(player.getInventory().getItemInMainHand())) {
        ...
        if (swordsManager.canUseCounterAttack(painSource)) {
          swordsManager.counterAttackChecks((LivingEntity)painSource,
                                            event.getDamage());
        }
      }
    }
    ...
    if (painSourceRoot instanceof Player && entityType == EntityType.PLAYER) {
      ...
      if (target instanceof Tameable) {
        if (heldItem.getType() == Material.BONE) {
          TamingManager tamingManager =
              UserManager.getPlayer(player).getTamingManager();
      ...
      if (tamer instanceof Player &&
          PrimarySkillType.TAMING.shouldProcess(target)) {
        Player master = (Player)tamer;

        if (!Misc.isNPCEntityExcludingVillagers(master) &&
            PrimarySkillType.TAMING.getPermissions(master)) {
  ...
  /**
   * Calculate and return the RAW damage bonus from Limit Break before
   * reductions
   ...
   *     reductions
   */
  public static int getLimitBreakDamage(Player attacker, LivingEntity defender,
                                        SubSkillType subSkillType) {
    if (defender instanceof Player) {
      Player playerDefender = (Player)defender;
      return getLimitBreakDamageAgainstQuality(
          attacker, subSkillType, getArmorQualityLevel(playerDefender));
    } else {
  ...
  /**
   * Calculate the RAW daamge value of limit break based on the armor quality of
   * the target PVE mobs are passed in with a value of 1000 for armor quality,
   * hacky... I'll change it later
   ...
   */
  public static int getLimitBreakDamageAgainstQuality(Player attacker,
                                                      SubSkillType subSkillType,
  ...
  /**
   * Get the quality level of the armor of a player used for Limit Break
   * calculations
  ...
  private static int getArmorQuality(ItemStack itemStack) {
    return mcMMO.getMaterialMapStore().getTier(
   ...
   */
  public static boolean canUseLimitBreak(Player player, LivingEntity target,
                                         SubSkillType subSkillType) {
    if (target instanceof Player ||
        AdvancedConfig.getInstance().canApplyLimitBreakPVE()) {
  ...
  public static void dealDamage(LivingEntity target, double damage,
    ...
    target.damage(
        getFakeDamageFinalResult(attacker, target, damage, modifiers));
  }
  ...
  public static void dealNoInvulnerabilityTickDamage(LivingEntity target,
                                                     double damage,
                                                     Entity attacker) {
    if (target.isDead()) {
      return;
    ...
    // TODO: This is horrible, but there is no cleaner way to do this without
    // potentially breaking existing code right now calling damage here is a
    // double edged sword: On one hand, without a call, plugins won't see this
    // properly when the entity dies, potentially mis-attributing the death
    // cause; calling a fake event would partially fix this, but this and
    // setting the last damage cause do have issues around plugin observability.
    // This is not a perfect solution, but it appears to be the best one here We
    // also set no damage ticks to 0, to ensure that damage is applied for this
    // case, and reset it back to the original value Snapshot current state so
    // we can pop up properly
    boolean wasMetaSet =
        target.getMetadata(mcMMO.CUSTOM_DAMAGE_METAKEY).size() != 0;
  ...
  public static void dealNoInvulnerabilityTickDamageRupture(LivingEntity target,
                                                            double damage,
                                                            Entity attacker,
                                                            int toolTier) {
    if (target.isDead()) {
    ...
    ////        int noDamageTicks = target.getNoDamageTicks();
    //
    ////        String debug = "BLEED DMG RESULT: INC DMG:"+damage+",
    ///HP-Before:"+target.getHealth()+", HP-After:";
    //
    ////        double incDmg = getFakeDamageFinalResult(attacker, target,
    ...
    ////            return;
    //
    //        target.setMetadata(mcMMO.CUSTOM_DAMAGE_METAKEY,
    //        mcMMO.metadataValue);
    //
    ...
    //        else
    //        {
    ////            Vector beforeRuptureVec = new
    ///Vector(target.getVelocity().getX(), target.getVelocity().getY(),
    ///target.getVelocity().getZ()); ;
    //            target.damage(damage, attacker);
    ////            debug+=target.getHealth();
    //            Bukkit.broadcastMessage(debug);
    ////            target.setNoDamageTicks(noDamageTicks); //Do not add
    ///additional IFrames /            target.setVelocity(beforeRuptureVec);
    //        }
  }
  ...
  public static void applyAbilityAoE(Player attacker, LivingEntity target,
    ...
    int numberOfTargets =
        getTier(attacker.getInventory()
                    .getItemInMainHand()); // The higher the weapon tier, the
                                           // more targets you hit
      ...
      case SWORDS:
        if (entity instanceof Player) {
          NotificationManager.sendPlayerInformation(
              (Player)entity, NotificationType.SUBSKILL_MESSAGE,
  ...
  public static void startGainXp(McMMOPlayer mcMMOPlayer, LivingEntity target,
   ...
   */
  private static void startGainXp(McMMOPlayer mcMMOPlayer, LivingEntity target,
                                  PrimarySkillType primarySkillType,
                                  double multiplier) {
    double baseXP = 0;
    ...
    if (baseXP != 0) {
      new AwardCombatXpTask(mcMMOPlayer, primarySkillType, baseXP, target,
                            xpGainReason)
          .runTaskLater(mcMMO.p, 0);
    }
  }
  ...
  private static boolean shouldBeAffected(Player player, Entity entity) {
    if (entity instanceof Player) {
      ...
      if ((PartyManager.inSameParty(player, defender) ||
           PartyManager.areAllies(player, defender)) &&
          !(Permissions.friendlyFire(player) &&
      ...
      // It may seem a bit redundant but we need a check here to prevent bleed
      // from being applied in applyAbilityAoE()
      ...
      if (isFriendlyPet(player, (Tameable)entity)) {
        ...
        // and the owner is in the same party So we can make some assumptions
        // here, about our casting and our check
        Player owner = (Player)((Tameable)entity).getOwner();
        return Permissions.friendlyFire(player) &&
            Permissions.friendlyFire(owner);
      }
    }
  ...
  public static boolean isInvincible(LivingEntity entity, double eventDamage) {
    ...
    return (entity.getNoDamageTicks() >
            entity.getMaximumNoDamageTicks() / 2.0F) &&
    ...
    if (pet.isTamed()) {
      ...
      if (tamer instanceof Player) {
        ...
        return (owner == attacker ||
                PartyManager.inSameParty(attacker, owner) ||
                PartyManager.areAllies(attacker, owner));
      }
    }
  ...
  @Deprecated
  public static double getFakeDamageFinalResult(Entity attacker, Entity target,
                                                double damage) {
    return getFakeDamageFinalResult(
        attacker, target, DamageCause.ENTITY_ATTACK,
        new EnumMap<DamageModifier, Double>(
            ImmutableMap.of(DamageModifier.BASE, damage)));
  }
  ...
  public static double getFakeDamageFinalResult(Entity attacker, Entity target,
    ...
    EntityDamageEvent damageEvent =
        sendEntityDamageEvent(attacker, target, damageCause, damage);
  ...
  public static EntityDamageEvent sendEntityDamageEvent(Entity attacker,
                                                        Entity target,
                                                        DamageCause damageCause,
                                                        double damage) {
    EntityDamageEvent damageEvent =
        attacker == null
            ? new FakeEntityDamageEvent(target, damageCause, damage)
            : new FakeEntityDamageByEntityEvent(attacker, target, damageCause,
                                                damage);
  ...
  getFakeDamageFinalResult(Entity attacker, Entity target,
                           Map<DamageModifier, Double> modifiers) {
    return getFakeDamageFinalResult(attacker, target, DamageCause.ENTITY_ATTACK,
                                    modifiers);
  }

  public static double
  getFakeDamageFinalResult(Entity attacker, Entity target, double damage,
                           Map<DamageModifier, Double> modifiers) {
    return getFakeDamageFinalResult(attacker, target, DamageCause.ENTITY_ATTACK,
  ...
  public static double
  getFakeDamageFinalResult(Entity attacker, Entity target, DamageCause cause,
                           Map<DamageModifier, Double> modifiers) {
    EntityDamageEvent damageEvent =
        attacker == null ? new FakeEntityDamageEvent(target, cause, modifiers)
  ...
  private static Map<DamageModifier, Double>
  getModifiers(EntityDamageEvent event) {
    Map<DamageModifier, Double> modifiers =
  ...
  private static Map<DamageModifier, Double>
  getScaledModifiers(double damage, Map<DamageModifier, Double> modifiers) {
    Map<DamageModifier, Double> scaledModifiers =
        new HashMap<DamageModifier, Double>();
  ...
  public static EntityDamageByEntityEvent
  applyScaledModifiers(double initialDamage, double finalDamage,
                       EntityDamageByEntityEvent event) {
  ...
  public static void handleHealthbars(Entity attacker, LivingEntity target,
    ...
    if (Misc.isNPCEntityExcludingVillagers(player) ||
        Misc.isNPCEntityExcludingVillagers(target)) {
      return;
    }
