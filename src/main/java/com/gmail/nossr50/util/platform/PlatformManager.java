...
public class PlatformManager {
  ...
  public PlatformManager() { init(); }

  private void init() { platform = loadPlatform(); }
  ...
  private @Nullable Platform loadPlatform() {
    ...
    MinecraftGameVersion gameVersion =
        determineGameVersion(Bukkit.getBukkitVersion());

    return platformBuilder.setMinecraftGameVersion(gameVersion)
  ...
  private @NotNull
  MinecraftGameVersion determineGameVersion(String platformVersionString) {
    ...
    // If the first character isn't a digit it's not a patch number and its some
    // crap we don't care about
    if (splitVersion.length > 2 &&
        Character.isDigit(splitVersion[2].toCharArray()[0]))
      patch = getSubsequentDigits(splitVersion[2].toCharArray(), 0);

    mcMMO.p.getLogger().info("Minecraft version determined to be - " + major +
                             "." + minor + "." + patch);
  ...
  private int getSubsequentDigits(char[] charArray, int position) {
    ...
    String digits = digitArrayList.stream()
                        .map(String::valueOf)
