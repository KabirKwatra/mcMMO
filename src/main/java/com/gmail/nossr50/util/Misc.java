...
public final class Misc {
  ...
  /*    public static final float ANVIL_USE_PITCH  = 0.3F;  // Not in CB
     directly, I went off the place sound values public static final float
     ANVIL_USE_VOLUME = 1.0F * Config.getInstance().getMasterVolume();  // Not
     in CB directly, I went off the place sound values public static final float
     FIZZ_VOLUME      = 0.5F * Config.getInstance().getMasterVolume(); public
     static final float POP_VOLUME       = 0.2F *
     Config.getInstance().getMasterVolume(); public static final float
     BAT_VOLUME       = 1.0F * Config.getInstance().getMasterVolume(); public
     static final float BAT_PITCH        = 0.6F; public static final float
     GHAST_VOLUME     = 1.0F * Config.getInstance().getMasterVolume(); public
     static final float LEVELUP_PITCH    = 0.5F;  // Reduced to differentiate
     between vanilla level-up
      public static final float LEVELUP_VOLUME   = 0.75F *
     Config.getInstance().getMasterVolume(); // Use max volume always*/

  public static final Set<String> modNames = ImmutableSet.of(
      "LOTR", "BUILDCRAFT", "ENDERIO", "ENHANCEDBIOMES", "IC2", "METALLURGY",
      "FORESTRY", "GALACTICRAFT", "RAILCRAFT", "TWILIGHTFOREST", "THAUMCRAFT",
      "GRAVESTONEMOD", "GROWTHCRAFT", "ARCTICMOBS", "DEMONMOBS", "INFERNOMOBS",
      "SWAMPMOBS", "MARICULTURE", "MINESTRAPPOLATION");

  private Misc(){};

  /**
   * Determines if an entity is an NPC but not a villager
   * This method aims to establish compatibility between mcMMO and other plugins
   * which create "NPCs"
   *
   ...
   *
   * In this context, an NPC is a bit hard to define. Various plugins determine
   * what an NPC is in different ways.
  ...
  public static boolean isVillager(Entity entity) {
    ...
    return entityType.equalsIgnoreCase("wandering_trader") || entity instanceof
                                                                  Villager;
  }

  public static boolean isNPCIncludingVillagers(Entity entity) {
    return (
        entity == null || (hasNPCMetadataTag(entity)) ||
   ...
   * @return true if the distance between {@code first} and {@code second} is
   *     less than {@code maxDistance}, false otherwise
   */
  public static boolean isNear(Location first, Location second,
                               double maxDistance) {
    return (first.getWorld() == second.getWorld()) &&
        (first.distanceSquared(second) < (maxDistance * maxDistance) ||
         maxDistance == 0);
  }
  ...
  public static Item dropItem(Location location, ItemStack itemStack) {
    ...
    // We can't get the item until we spawn it and we want to make it
    // cancellable, so we have a custom event.
   ...
   * @param quantity The amount of items to drop
   */
  public static void spawnItemsTowardsLocation(Location fromLocation,
                                               Location toLocation,
                                               ItemStack is, int quantity,
                                               double speed) {
    for (int i = 0; i < quantity; i++) {
  ...
  /**
   * Drop an item at a given location.
   * This method is fairly expensive as it creates clones of everything passed
   * to itself since they are mutable objects
   *
   ...
   * @return Dropped Item entity or null if invalid or cancelled
   */
  public static Item spawnItemTowardsLocation(Location fromLocation,
                                              Location toLocation,
                                              ItemStack itemToSpawn,
                                              double speed) {
    ...
    // We can't get the item until we spawn it and we want to make it
    // cancellable, so we have a custom event.
    McMMOItemSpawnEvent event =
        new McMMOItemSpawnEvent(spawnLocation, clonedItem);
    mcMMO.p.getServer().getPluginManager().callEvent(event);
    ...
    Item spawnedItem =
        spawnLocation.getWorld().dropItem(spawnLocation, clonedItem);
  ...
  public static void profileCleanup(String playerName) {
    ...
    if (player != null) {
      ...
      new PlayerProfileLoadingTask(player).runTaskLaterAsynchronously(
          mcMMO.p, 1); // 1 Tick delay to ensure the player is marked as online
                       // before we begin loading
    }
  }

  public static void printProgress(int convertedUsers, int progressInterval,
                                   long startMillis) {
    if ((convertedUsers % progressInterval) == 0) {
      mcMMO.p.getLogger().info(String.format(
          "Conversion progress: %d users at %.2f users/second", convertedUsers,
