...
public class RepairConfig extends ConfigLoader {
  ...
  protected void loadKeys() {
    ...
    if (!config.isConfigurationSection("Repairables")) {
      mcMMO.p.getLogger().severe("Could not find Repairables section in " +
                                 fileName);
      return;
    }

    ConfigurationSection section =
        config.getConfigurationSection("Repairables");
    ...
    for (String key : keys) {
      ...
      if (itemMaterial == null) {
        // mcMMO.p.getLogger().info("No support for repair item "+key+ " in this
        // version of Minecraft, skipping.");
      ...
      if (!config.contains("Repairables." + key + ".MaterialType") &&
        ...
        } else if (ItemUtils.isIronArmor(repairItem) ||
                   ItemUtils.isIronTool(repairItem)) {
          repairMaterialType = MaterialType.IRON;
        } else if (ItemUtils.isGoldArmor(repairItem) ||
                   ItemUtils.isGoldTool(repairItem)) {
          repairMaterialType = MaterialType.GOLD;
        } else if (ItemUtils.isDiamondArmor(repairItem) ||
                   ItemUtils.isDiamondTool(repairItem)) {
        ...
        try {
          ...
          reason.add(key + " has an invalid MaterialType of " +
                     repairMaterialTypeString);
        }
      }

      // Repair Material
      String repairMaterialName =
          config.getString("Repairables." + key + ".RepairMaterial");
      ...
      short maximumDurability =
          (itemMaterial != null ? itemMaterial.getMaxDurability()
                                : (short)config.getInt("Repairables." + key +
                                                       ".MaximumDurability"));
      ...
      String repairItemTypeString =
          config.getString("Repairables." + key + ".ItemType", "OTHER");

      if (!config.contains("Repairables." + key + ".ItemType") &&
          itemMaterial != null) {
        ItemStack repairItem = new ItemStack(itemMaterial);
        ...
        try {
          ...
          reason.add(key + " has an invalid ItemType of " +
                     repairItemTypeString);
        }
      }

      int minimumLevel = config.getInt("Repairables." + key + ".MinimumLevel");
      double xpMultiplier =
          config.getDouble("Repairables." + key + ".XpMultiplier", 1);
      ...
      // Minimum Quantity
      int minimumQuantity =
          config.getInt("Repairables." + key + ".MinimumQuantity");
      ...
      if (noErrorsInRepairable(reason)) {
        Repairable repairable = RepairableFactory.getRepairable(
            ...
            repairItemType, repairMaterialType, xpMultiplier, minimumQuantity);
