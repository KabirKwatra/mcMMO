...
public class SwordsManager extends SkillManager {
  ...
  public boolean canUseSerratedStrike() {
    if (!RankUtils.hasUnlockedSubskill(getPlayer(),
                                       SubSkillType.SWORDS_SERRATED_STRIKES))
      return false;
  ...
  public void ruptureCheck(LivingEntity target) {
    if (RandomChanceUtil.isActivationSuccessful(
            ...
            SubSkillType.SWORDS_RUPTURE, getPlayer(),
            this.mcMMOPlayer.getAttackStrength())) {
      ...
      BleedTimerTask.add(
          target, getPlayer(), getRuptureBleedTicks(),
          RankUtils.getRank(getPlayer(), SubSkillType.SWORDS_RUPTURE),
          getToolTier(getPlayer().getInventory().getItemInMainHand()));
  ...
  public int getRuptureBleedTicks() {
    int bleedTicks =
        2 * RankUtils.getRank(getPlayer(), SubSkillType.SWORDS_RUPTURE);
  ...
  public void counterAttackChecks(LivingEntity attacker, double damage) {
    if (RandomChanceUtil.isActivationSuccessful(
            SkillActivationType.RANDOM_LINEAR_100_SCALE_WITH_CAP,
            SubSkillType.SWORDS_COUNTER_ATTACK, getPlayer())) {
      ...
      if (attacker instanceof Player) {
        NotificationManager.sendPlayerInformation(
            (Player)attacker, NotificationType.SUBSKILL_MESSAGE,
  ...
  public void serratedStrikes(LivingEntity target, double damage,
    ...
    CombatUtils.applyAbilityAoE(getPlayer(), target,
                                damage / Swords.serratedStrikesModifier,
                                modifiers, skill);
  }
}
