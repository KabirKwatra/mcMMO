...
public class FishingManager extends SkillManager {
  ...
  public boolean canMasterAngler() {
    return getSkillLevel() >=
        RankUtils.getUnlockLevel(SubSkillType.FISHING_MASTER_ANGLER) &&
  ...
  public void setFishingRodCastTimestamp() {
    ...
    if (currentTime <
        fishingRodCastTimestamp + FISHING_ROD_CAST_CD_MILLISECONDS) {
      ...
      if (lastWarnedExhaust + (1000 * 1) < currentTime) {
        ...
        SoundManager.sendSound(getPlayer(), getPlayer().getLocation(),
                               SoundType.TIRED);
      }
    }
  ...
  public boolean isExploitingFishing(Vector centerOfCastVector) {
    ...
    boolean sameTarget = lastFishingBoundingBox != null &&
                         lastFishingBoundingBox.overlaps(newCastBoundingBox);
    ...
    if (fishCaughtCounter + 1 == OVERFISH_LIMIT) {
      getPlayer().sendMessage(
          LocaleLoader.getString("Fishing.LowResourcesTip", 3));
    }

    // If the new bounding box does not intersect with the old one, then update
    // our bounding box reference
    if (!sameTarget)
  ...
  public boolean canIceFish(Block block) {
    if (getSkillLevel() <
        RankUtils.getUnlockLevel(SubSkillType.FISHING_ICE_FISHING)) {
      return false;
    }
  ...
  protected int getVanillaXPBoostModifier() {
    return AdvancedConfig.getInstance().getFishingVanillaXPModifier(
        getLootTier());
  }
   ...
   */
  public double getShakeProbability() { return getShakeChance(); }
  ...
  public void masterAngler(FishHook hook) {
    ...
    if (Fishing.masterAnglerBiomes.contains(location.getBlock().getBiome())) {
      biteChance = biteChance *
                   AdvancedConfig.getInstance().getMasterAnglerBiomeModifier();
    }

    if (player.isInsideVehicle() &&
        player.getVehicle().getType() == EntityType.BOAT) {
      biteChance = biteChance *
                   AdvancedConfig.getInstance().getMasterAnglerBoatModifier();
    }
  ...
  public boolean isMagicHunterEnabled() {
    return RankUtils.hasUnlockedSubskill(getPlayer(),
  ...
  public void handleFishing(Item fishingCatch) {
    ...
    int fishXp = ExperienceConfig.getInstance().getXp(
        PrimarySkillType.FISHING, fishingCatch.getItemStack().getType());
    int treasureXp = 0;
    Player player = getPlayer();
    FishingTreasure treasure = null;

    if (Config.getInstance().getFishingDropsEnabled() &&
        Permissions.isSubSkillEnabled(player,
                                      SubSkillType.FISHING_TREASURE_HUNTER)) {
    ...
    if (treasure != null) {
      ItemStack treasureDrop =
          treasure.getDrop().clone(); // Not cloning is bad, m'kay?
      ...
      McMMOPlayerFishingTreasureEvent event =
          EventUtils.callFishingTreasureEvent(player, treasureDrop,
      ...
      // Drop the original catch at the feet of the player and set the treasure
      // as the real catch
  ...
  public void shakeCheck(LivingEntity target) {
    if (RandomChanceUtil.checkRandomChanceExecutionSuccess(
            new RandomChanceSkillStatic(getShakeChance(), getPlayer(),
                                        SubSkillType.FISHING_SHAKE))) {
      ...
      // It's possible that chooseDrop returns null if the sum of probability in
      // possibleDrops is inferior than 100
      if (drop == null) {
        return;
      }
        ...
        case BEDROCK:
          if (TreasureConfig.getInstance().getInventoryStealEnabled()) {
            ...
            if (TreasureConfig.getInstance().getInventoryStealStacks()) {
              ...
              inventory.setItem(slot, (drop.getAmount() > 1)
                                          ? new ItemStack(drop.getType(),
                                                          drop.getAmount() - 1)
                                          : null);
              drop.setAmount(1);
            }
      ...
      McMMOPlayerShakeEvent shakeEvent =
      ...
      CombatUtils.dealDamage(
          target, Math.min(Math.max(target.getMaxHealth() / 4, 1), 10),
          EntityDamageEvent.DamageCause.CUSTOM,
          getPlayer()); // Make it so you can shake a mob no more than 4 times.
  ...
  private FishingTreasure getFishingTreasure() {
    ...
    if (getPlayer().getInventory().getItemInMainHand().getType() ==
        Material.FISHING_ROD) {
      luck = getPlayer().getInventory().getItemInMainHand().getEnchantmentLevel(
          Enchantment.LUCK);
    } else {
      // We know something was caught, so if the rod wasn't in the main hand it
      // must be in the offhand
      luck = getPlayer().getInventory().getItemInOffHand().getEnchantmentLevel(
          Enchantment.LUCK);
    }

    // Rather than subtracting luck (and causing a minimum 3% chance for every
    // drop), scale by luck.
    diceRoll *=
        (1.0 - luck * Config.getInstance().getFishingLureModifier() / 100);

    FishingTreasure treasure = null;

    for (Rarity rarity : Rarity.values()) {
      double dropRate =
          TreasureConfig.getInstance().getItemDropRate(getLootTier(), rarity);
      ...
      if (diceRoll <= dropRate) {
        ...
        List<FishingTreasure> fishingTreasures =
            TreasureConfig.getInstance().fishingRewards.get(rarity);

        if (fishingTreasures.isEmpty()) {
          return null;
        }

        treasure = fishingTreasures.get(
            Misc.getRandom().nextInt(fishingTreasures.size()));
        break;
      }
    ...
    if (maxDurability > 0) {
      treasureDrop.setDurability(
          (short)(Misc.getRandom().nextInt(maxDurability)));
    }

    // TODO: Add option to randomize the amount rewarded
    /*if (treasureDrop.getAmount() > 1) {
        treasureDrop.setAmount(Misc.getRandom().nextInt(treasureDrop.getAmount())
    + 1);
    }*/
  ...
  private Map<Enchantment, Integer> handleMagicHunter(ItemStack treasureDrop) {
    ...
    for (Rarity rarity : Rarity.values()) {
      ...
      double dropRate = TreasureConfig.getInstance().getEnchantmentDropRate(
          getLootTier(), rarity);

      if (diceRoll <= dropRate) {
        // Make sure enchanted books always get some kind of enchantment.
        // --hoorigan
        ...
        fishingEnchantments =
            TreasureConfig.getInstance().fishingEnchantments.get(rarity);
        break;
      }
    ...
    List<EnchantmentTreasure> possibleEnchants =
        new ArrayList<EnchantmentTreasure>();
   ...
   */
  private int getVanillaXpMultiplier() { return getVanillaXPBoostModifier(); }
}
