...
public class UnarmedManager extends SkillManager {
  ...
  public boolean canActivateAbility() {
    return mcMMOPlayer.getToolPreparationMode(ToolType.FISTS) &&
        Permissions.berserk(getPlayer());
  }
  ...
  public boolean canUseIronArm() {
    ...
    return Permissions.isSubSkillEnabled(getPlayer(),
                                         SubSkillType.UNARMED_IRON_ARM_STYLE);
  }
  ...
  public boolean canDisarm(LivingEntity target) {
    ...
    return target instanceof Player &&
        ((Player)target).getInventory().getItemInMainHand().getType() !=
            Material.AIR &&
  ...
  public boolean blockCrackerCheck(BlockState blockState) {
    if (!RandomChanceUtil.isActivationSuccessful(
            SkillActivationType.ALWAYS_FIRES,
            SubSkillType.UNARMED_BLOCK_CRACKER, getPlayer())) {
      return false;
    }
  ...
  public boolean deflectCheck() {
    if (RandomChanceUtil.isActivationSuccessful(
            SkillActivationType.RANDOM_LINEAR_100_SCALE_WITH_CAP,
            SubSkillType.UNARMED_ARROW_DEFLECT, getPlayer())) {
      NotificationManager.sendPlayerInformation(
          getPlayer(), NotificationType.SUBSKILL_MESSAGE,
          "Combat.ArrowDeflect");
      return true;
    }
  ...
  public double berserkDamage(double damage) {
    damage = ((damage * Unarmed.berserkDamageModifier) *
              mcMMOPlayer.getAttackStrength()) -
             damage;
  ...
  public double calculateIronArmDamage() {
    if (!RandomChanceUtil.isActivationSuccessful(
            SkillActivationType.ALWAYS_FIRES,
            SubSkillType.UNARMED_IRON_ARM_STYLE, getPlayer())) {
      return 0;
    }
  ...
  public double getIronArmDamage() {
    int rank =
        RankUtils.getRank(getPlayer(), SubSkillType.UNARMED_IRON_ARM_STYLE);
  ...
  private boolean hasIronGrip(Player defender) {
    if (!Misc.isNPCEntityExcludingVillagers(defender) &&
        Permissions.isSubSkillEnabled(defender,
                                      SubSkillType.UNARMED_IRON_GRIP) &&
        RandomChanceUtil.isActivationSuccessful(
            SkillActivationType.RANDOM_LINEAR_100_SCALE_WITH_CAP,
            SubSkillType.UNARMED_IRON_GRIP, defender)) {
      NotificationManager.sendPlayerInformation(
