...
public class MiningManager extends SkillManager {
  ...
  public boolean canUseDemolitionsExpertise() {
    if (!RankUtils.hasUnlockedSubskill(
            getPlayer(), SubSkillType.MINING_DEMOLITIONS_EXPERTISE))
      return false;
    ...
    return canUseBlastMining() && player.isSneaking() &&
        (ItemUtils.isPickaxe(getPlayer().getInventory().getItemInMainHand()) ||
         player.getInventory().getItemInMainHand().getType() ==
             Config.getInstance().getDetonatorItem()) &&
  ...
  public void miningBlockCheck(BlockState blockState) {
    ...
    if (mcMMOPlayer.getAbilityMode(skill.getAbility())) {
      SkillUtils.handleDurabilityChange(
          ...
          Config.getInstance().getAbilityToolDamage());
    }
    ...
    boolean silkTouch =
        player.getInventory().getItemInMainHand().containsEnchantment(
            Enchantment.SILK_TOUCH);

    if (silkTouch &&
        !AdvancedConfig.getInstance().getDoubleDropSilkTouchEnabled())
      return;

    // TODO: Make this readable
    if (RandomChanceUtil.checkRandomChanceExecutionSuccess(
            getPlayer(), SubSkillType.MINING_DOUBLE_DROPS, true)) {
      BlockUtils.markDropsAsBonus(
          blockState, mcMMOPlayer.getAbilityMode(skill.getAbility()));
    }
  }
  ...
  public void remoteDetonation() {
    Player player = getPlayer();
    Block targetBlock =
        player.getTargetBlock(BlockUtils.getTransparentBlocks(),
                              BlastMining.MAXIMUM_REMOTE_DETONATION_DISTANCE);

    // Blast mining cooldown check needs to be first so the player can be
    // messaged
    if (!blastMiningCooldownOver() || targetBlock.getType() != Material.TNT ||
        !EventUtils.simulateBlockBreak(targetBlock, player, true)) {
      return;
    }

    TNTPrimed tnt =
        player.getWorld().spawn(targetBlock.getLocation(), TNTPrimed.class);

    // SkillUtils.sendSkillMessage(player,
    // SuperAbilityType.BLAST_MINING.getAbilityPlayer(player));
    NotificationManager.sendPlayerInformation(
        player, NotificationType.SUPER_ABILITY, "Mining.Blast.Boom");
    ...
    mcMMOPlayer.setAbilityDATS(SuperAbilityType.BLAST_MINING,
                               System.currentTimeMillis());
    mcMMOPlayer.setAbilityInformed(SuperAbilityType.BLAST_MINING, false);
    new AbilityCooldownTask(mcMMOPlayer, SuperAbilityType.BLAST_MINING)
        .runTaskLater(mcMMO.p, SuperAbilityType.BLAST_MINING.getCooldown() *
  ...
  public void blastMiningDropProcessing(float yield, EntityExplodeEvent event) {
    ...
    for (Block targetBlock : event.blockList()) {
      // Containers usually have 0 XP unless someone edited their config in a
      // very strange way
      if (ExperienceConfig.getInstance().getXp(PrimarySkillType.MINING,
                                               targetBlock) == 0 ||
    ...
    for (BlockState blockState : ores) {
      if (RandomUtils.nextInt(ores.size()) >= (ores.size() / 2)) {
        ...
        Misc.dropItem(
            ...
            new ItemStack(blockState.getType())); // Initial block that would
                                                  // have been dropped

        for (int i = 1; i < dropMultiplier; i++) {
          if (RandomUtils.nextInt(100) >= 75)
            Mining.handleSilkTouchDrops(
                blockState); // Bonus drops - should drop the block & not the
                             // items
        }
      }
    }
   ...
   * @return the Blast Mining tier
   */
  public double getOreBonus() { return getOreBonus(getBlastMiningTier()); }
  ...
  private boolean blastMiningCooldownOver() {
    int timeRemaining =
        mcMMOPlayer.calculateTimeRemaining(SuperAbilityType.BLAST_MINING);

    if (timeRemaining > 0) {
      // getPlayer().sendMessage(LocaleLoader.getString("Skills.TooTired",
      // timeRemaining));
      NotificationManager.sendPlayerInformation(
          getPlayer(), NotificationType.ABILITY_COOLDOWN, "Skills.TooTired",
