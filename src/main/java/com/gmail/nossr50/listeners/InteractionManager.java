...
public class InteractionManager {
  ...
  private static HashMap<String, AbstractSubSkill>
      subSkillNameMap; // Used for mmoinfo optimization
  ...
  public static void registerSubSkill(AbstractSubSkill abstractSubSkill) {
    ...
    if (interactRegister.get(abstractSubSkill.getInteractType()) == null)
      interactRegister.put(abstractSubSkill.getInteractType(),
                           new ArrayList<>());

    // Registration array reference
    ArrayList<Interaction> arrayRef =
        interactRegister.get(abstractSubSkill.getInteractType());

    // Register skill
    arrayRef.add(abstractSubSkill);

    String lowerCaseName =
        abstractSubSkill.getConfigKeyName().toLowerCase(Locale.ENGLISH);

    // Register in name map
    if (subSkillNameMap.get(lowerCaseName) == null)
      subSkillNameMap.put(lowerCaseName, abstractSubSkill);

    mcMMO.p.getLogger().info("Registered subskill: " +
   ...
   */
  public static void processEvent(Event event, mcMMO plugin,
                                  InteractType curInteractType) {
  ...
  /**
   * Returns the list that contains all unique instances of registered
   * Interaction classes Interactions are extensions of abstract classes that
   * represent modifying behaviours in Minecraft through events
   ...
   */
  public static HashMap<InteractType, ArrayList<Interaction>>
  getInteractRegister() {
