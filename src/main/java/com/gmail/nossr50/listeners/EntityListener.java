...
public class EntityListener implements Listener {
  ...
  public EntityListener(final mcMMO plugin) { this.plugin = plugin; }

  @EventHandler(priority = EventPriority.MONITOR)
  public void onEntityTransform(EntityTransformEvent event) {
    // Transfer metadata keys from mob-spawned mobs to new mobs
    if (event.getEntity().getMetadata(mcMMO.entityMetadataKey) != null ||
        event.getEntity().getMetadata(mcMMO.entityMetadataKey).size() >= 1) {
  ...
  public void onEntityTargetEntity(EntityTargetLivingEntityEvent event) {
    ...
    if (event.getTarget() instanceof Endermite) {
      if (!event.getEntity().hasMetadata(mcMMO.entityMetadataKey))
        event.getEntity().setMetadata(mcMMO.entityMetadataKey,
                                      mcMMO.metadataValue);
    }
  }
  ...
  public void onEntityShootBow(EntityShootBowEvent event) {
    ...
    // Should be noted that there are API changes regarding Arrow from 1.13.2 to
    ...
    projectile.setMetadata(
        ...
        new FixedMetadataValue(
            ...
            Math.min(event.getForce() *
                         AdvancedConfig.getInstance().getForceMultiplier(),
                     1.0)));
    projectile.setMetadata(
        mcMMO.arrowDistanceKey,
        new FixedMetadataValue(plugin, projectile.getLocation()));
  }
  ...
  public void onProjectileLaunch(ProjectileLaunchEvent event) {
    ...
    if (event.getEntity().getShooter() instanceof Player) {
      ...
      projectile.setMetadata(mcMMO.bowForceKey,
                             new FixedMetadataValue(plugin, 1.0));
      projectile.setMetadata(
          mcMMO.arrowDistanceKey,
          new FixedMetadataValue(plugin, projectile.getLocation()));

      for (Enchantment enchantment : player.getInventory()
                                         .getItemInMainHand()
                                         .getEnchantments()
                                         .keySet()) {
      ...
      if (RandomChanceUtil.isActivationSuccessful(
              SkillActivationType.RANDOM_LINEAR_100_SCALE_WITH_CAP,
              SubSkillType.ARCHERY_ARROW_RETRIEVAL, player)) {
  ...
  public void onEntityCombustByEntityEvent(EntityCombustByEntityEvent event) {
    // Prevent players from setting fire to each other if they are in the same
    // party
    if (event.getEntity() instanceof Player) {
  ...
  public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
    ...
    if (WorldGuardUtils.isWorldGuardLoaded()) {
      if (attacker instanceof Player) {
        ...
        if (projectile.getShooter() instanceof Player) {
          if (!WorldGuardManager.getInstance().hasMainFlag(
    ...
    // Don't process this event for marked entities, for players this is handled
    // above, However, for entities, we do not wanna cancel this event to allow
    // plugins to observe changes properly
    ...
    if (Misc.isNPCEntityExcludingVillagers(defender) || !defender.isValid() ||
    ...
    if (defender instanceof Player) {
      ...
      if (attacker instanceof Projectile || attacker instanceof Player) {
        if (attacker instanceof Projectile) {
          ...
          // Deflect checks
          final McMMOPlayer mcMMOPlayer =
              UserManager.getPlayer(defendingPlayer);
          if (mcMMOPlayer != null) {
    ...
    /*
     * This was put here to solve a plugin conflict with a mod called Project
     * Korra Project Korra sends out a damage event with exactly 0 damage mcMMO
     * does some calculations for the damage in an event and it ends up dividing
     * by zero, as a result of the modifiers for the event being 0 and the
     * damage set for this event being 0.
     *
  ...
  public void
  onEntityDamageMonitor(EntityDamageByEntityEvent entityDamageEvent) {
  ...
  public boolean checkParties(Cancellable event, Player defendingPlayer,
                              Player attackingPlayer) {
    if (!UserManager.hasPlayerDataKey(defendingPlayer) ||
  ...
  public void onEntityDamage(EntityDamageEvent event) {
    ...
    InteractionManager.processEvent(event, plugin,
                                    InteractType.ON_ENTITY_DAMAGE);
    ...
    /*
    As far as I can tell at one point we registered meta-data about custom
    damage and we no longer do that. if
    (entity.hasMetadata(mcMMO.customDamageKey)) {
        entity.removeMetadata(mcMMO.customDamageKey, plugin);
        return;
    }
    */

    if (Misc.isNPCEntityExcludingVillagers(entity) || !entity.isValid() ||
  ...
  public void onEntityBreed(EntityBreedEvent event) {
    if (ExperienceConfig.getInstance().isCOTWBreedingPrevented()) {
      if (event.getFather().hasMetadata(mcMMO.COTW_TEMPORARY_SUMMON) ||
        ...
        if (event.getBreeder() instanceof Player) {
          ...
          NotificationManager.sendPlayerInformationChatOnly(
              player, "Taming.Summon.COTW.BreedingDisallowed");
        }
      }
    }
  }
  ...
  public void onExplosionPrime(ExplosionPrimeEvent event) {
    ...
    if (!(entity instanceof TNTPrimed) ||
        !entity.hasMetadata(mcMMO.tntMetadataKey)) {
      return;
    }
    ...
    Player player = plugin.getServer().getPlayerExact(
        entity.getMetadata(mcMMO.tntMetadataKey).get(0).asString());
    ...
    MiningManager miningManager =
        UserManager.getPlayer(player).getMiningManager();
  ...
  public void onFoodLevelChange(FoodLevelChangeEvent event) {
    ...
    // The main hand is used over the off hand if they both have food, so check
    // the main hand first
    Material foodInHand;

    if (mcMMO.getMaterialMapStore().isFood(
            player.getInventory().getItemInMainHand().getType())) {
      foodInHand = player.getInventory().getItemInMainHand().getType();
    } else if (mcMMO.getMaterialMapStore().isFood(
    ...
    case PUMPKIN_PIE:   /*
      ...
      if (Permissions.isSubSkillEnabled(player,
        ...
        event.setFoodLevel(
            UserManager.getPlayer(player).getHerbalismManager().farmersDiet(
                newFoodLevel));
      }
      return;
    ...
    case COOKED_SALMON:
      ...
      if (Permissions.isSubSkillEnabled(player,
        ...
        event.setFoodLevel(UserManager.getPlayer(player)
                               .getFishingManager()
  ...
  public void onEntityTarget(EntityTargetEvent event) {
    ...
    if (!UserManager.hasPlayerDataKey(player) ||
        !CombatUtils.isFriendlyPet(player, tameable)) {
      return;
    }
  ...
  public void onPotionSplash(PotionSplashEvent event) {
    ...
    for (PotionEffect effect :
         ((PotionMeta)event.getPotion().getItem().getItemMeta())
             .getCustomEffects()) {
      ...
      for (LivingEntity entity : event.getAffectedEntities()) {
        ...
        entity.addPotionEffect(new PotionEffect(effect.getType(), duration,
                                                effect.getAmplifier(),
                                                effect.isAmbient()));
      }
    }
  ...
  public void onPigZapEvent(PigZapEvent event) {
    ...
    if (event.getEntity().hasMetadata(mcMMO.entityMetadataKey)) {
      event.getPigZombie().setMetadata(mcMMO.entityMetadataKey,
                                       mcMMO.metadataValue);
    }
  }
}
