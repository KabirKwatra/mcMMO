...
public class PlayerListener implements Listener {
  ...
  public PlayerListener(final mcMMO plugin) { this.plugin = plugin; }
  ...
  public void onPlayerTeleport(PlayerTeleportEvent event) {
    ...
    if (WorldBlacklist.isWorldBlacklisted(event.getPlayer().getWorld())) {
      ...
      // This only fires if they are traveling to a non-blacklisted world from a
      // blacklisted world
    ...
    if (!UserManager.hasPlayerDataKey(player) ||
        Config.getInstance().getXPAfterTeleportCooldown() <= 0 ||
  ...
  public void onPlayerDeathMonitor(PlayerDeathEvent event) {
    ...
    if (!killedPlayer.hasMetadata(mcMMO.playerDataKey) ||
  ...
  public void onPlayerFishHighest(PlayerFishEvent event) {
    ...
    if (!UserManager.hasPlayerDataKey(player) ||
        !PrimarySkillType.FISHING.getPermissions(player)) {
      return;
    }
    ...
    FishingManager fishingManager =
        UserManager.getPlayer(player).getFishingManager();

    switch (event.getState()) {
    case CAUGHT_FISH:
      // TODO Update to new API once available! Waiting for case
      // CAUGHT_TREASURE:
  ...
  private void cancelFishingEventAndDropXp(PlayerFishEvent event,
                                           Player player) {
    event.setCancelled(true);
    ExperienceOrb experienceOrb = (ExperienceOrb)player.getWorld().spawnEntity(
        player.getEyeLocation(), EntityType.EXPERIENCE_ORB);
  ...
  public void onPlayerFishMonitor(PlayerFishEvent event) {
    ...
    if (ExperienceConfig.getInstance().isFishingExploitingPrevented()) {
      if (event.getHook().getMetadata(mcMMO.FISH_HOOK_REF_METAKEY).size() ==
          0) {
    ...
    if (event.getEntity() instanceof Player) {
      ...
      if (drop.hasMetadata(mcMMO.disarmedItemKey)) {
        if (!player.getName().equals(
                drop.getMetadata(mcMMO.disarmedItemKey).get(0).asString())) {
          event.setCancelled(true);
        }
      ...
      if (!drop.hasMetadata(mcMMO.droppedItemKey) && mcMMOPlayer.inParty() &&
  ...
  public void onPlayerQuit(PlayerQuitEvent event) {
    ...
    // Basically there are conditions in which an async task does not execute
    // fast enough to save the data if the server shutdown shortly after this
    // task was scheduled
  ...
  public void onPlayerJoin(PlayerJoinEvent event) {
    ...
    // Delay loading for 3 seconds in case the player has a save task running,
    // its hacky but it should do the trick
    new PlayerProfileLoadingTask(player).runTaskLaterAsynchronously(mcMMO.p,
                                                                    60);
    ...
    if (plugin.isXPEventEnabled() &&
        Config.getInstance().playerJoinEventInfo()) {
  ...
  public void onPlayerInteractLowest(PlayerInteractEvent event) {
    ...
    if (clickedBlock != null) {
      ...
      if (clickedBlockType == Repair.anvilMaterial ||
          clickedBlockType == Salvage.anvilMaterial) {
        event.setUseItemInHand(Event.Result.ALLOW);

        if (mcMMO.getMaterialMapStore().isToolActivationBlackListed(
                clickedBlockType)) {
    ...
    case RIGHT_CLICK_BLOCK:
      ...
      if (!Config.getInstance().getAbilitiesOnlyActivateWhenSneaking() ||
        ...
        if (type == Repair.anvilMaterial &&
          ...
          // Make sure the player knows what he's doing when trying to repair an
          // enchanted item
        ...
        /* SALVAGE CHECKS */
                 ...
                 RankUtils.hasUnlockedSubskill(
                     player, SubSkillType.SALVAGE_SCRAP_COLLECTOR) &&
                 mcMMO.getSalvageableManager().isSalvageable(heldItem) &&
                 heldItem.getAmount() <= 1) {
          SalvageManager salvageManager =
              UserManager.getPlayer(player).getSalvageManager();
          event.setCancelled(true);

          // Make sure the player knows what he's doing when trying to salvage
          // an enchanted item
      ...
      /* BLAST MINING CHECK */
        ...
        if (type == Material.TNT) {
          event.setCancelled(
              true); // Don't detonate the TNT if they're too close
        } else {
  ...
  public void onPlayerInteractMonitor(PlayerInteractEvent event) {
    ...
    if (event.getAction() == Action.RIGHT_CLICK_BLOCK ||
      ...
      if (ExperienceConfig.getInstance().isFishingExploitingPrevented() &&
        ...
        if (player.isInsideVehicle() &&
          ...
          player.setVelocity(
              player.getEyeLocation().getDirection().multiply(10));
        }
    ...
    case RIGHT_CLICK_BLOCK:
      ...
      FakePlayerAnimationEvent fakeSwing = new FakePlayerAnimationEvent(
          event.getPlayer()); // PlayerAnimationEvent compat
      if (herbalismManager.canGreenThumbBlock(blockState)) {
        Bukkit.getPluginManager().callEvent(fakeSwing);
        player.getInventory().setItemInMainHand(
            new ItemStack(Material.WHEAT_SEEDS, heldItem.getAmount() - 1));
    ...
    case LEFT_CLICK_BLOCK:
      ...
      if (type == Config.getInstance().getTamingCOTWMaterial(
                      CallOfTheWildType.WOLF.getConfigEntityTypeEntry())) {
        tamingManager.summonWolf();
      } else if (type ==
                 Config.getInstance().getTamingCOTWMaterial(
                     CallOfTheWildType.CAT.getConfigEntityTypeEntry())) {
  ...
  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void
  onPlayerStatisticIncrementEvent(PlayerStatisticIncrementEvent event) {
    /* WORLD BLACKLIST CHECK */
    if (WorldBlacklist.isWorldBlacklisted(event.getPlayer().getWorld()))
      return;
