...
public class mcMMO extends JavaPlugin {
  ...
  public final static String databaseCommandKey =
      "mcMMO: Processing Database Command";
  ...
  public void onEnable() {
    try {
      ...
      if (getServer().getName().equals("Cauldron") ||
          getServer().getName().equals("MCPC+")) {
        checkModConfigs();
      }
      ...
      if (pluginManager.getPlugin("NoCheatPlus") != null &&
          pluginManager.getPlugin("CompatNoCheatPlus") == null) {
        getLogger().warning(
      ...
      if (serverAPIOutdated) {
        ...
        if (platformManager.getServerSoftware() ==
            ServerSoftwareType.CRAFT_BUKKIT) {
          Bukkit.getScheduler().scheduleSyncRepeatingTask(
              this,
              ()
                  -> getLogger().severe(
        ...
        for (Player player : getServer().getOnlinePlayers()) {
          new PlayerProfileLoadingTask(player).runTaskLaterAsynchronously(
              mcMMO.p, 1); // 1 Tick delay to ensure the player is marked as
                           // online before we begin loading
        }
        ...
        placeStore =
            ChunkManagerFactory.getChunkManager(); // Get our ChunkletManager
      ...
      if (Config.getInstance().getIsMetricsEnabled()) {
        ...
        if (Config.getInstance().getIsRetroMode())
          metrics.addCustomChart(
              new Metrics.SimplePie("scaling", () -> "Standard"));
        else
          metrics.addCustomChart(
              new Metrics.SimplePie("scaling", () -> "Retro"));
      }
    } catch (Throwable t) {
  ...
  private void checkForOutdatedAPI() {
    try {
      ...
      Class<?> checkForClassBaseComponent =
  ...
  public void onDisable() {
    try {
      UserManager.saveAll(); // Make sure to save player information if the
                             // server shuts down
      UserManager.clearAll();
      Alchemy.finishAllBrews();   // Finish all partially complete
                                  // AlchemyBrewTasks to prevent vanilla brewing
                                  // continuation on restart
    ...
    if (Config.getInstance().getBackupsEnabled()) {
      // Remove other tasks BEFORE starting the Backup, or we just cancel it
      // straight away.
      try {
  ...
  public static String getMainDirectory() { return mainDirectory; }

  public static String getLocalesDirectory() { return localesDirectory; }

  public static String getFlatFileDirectory() { return flatFileDirectory; }

  public static String getUsersFilePath() { return usersFile; }

  public static String getModDirectory() { return modDirectory; }

  public boolean isXPEventEnabled() { return xpEventEnabled; }

  public void setXPEventEnabled(boolean enabled) {
    this.xpEventEnabled = enabled;
  }

  public void toggleXpEventEnabled() { xpEventEnabled = !xpEventEnabled; }
  ...
  public static HolidayManager getHolidayManager() { return holidayManager; }

  public static ChunkManager getPlaceStore() { return placeStore; }
  ...
  private void fixFilePaths() {
    File oldFlatfilePath =
        new File(mainDirectory + "FlatFileStuff" + File.separator);
    ...
    if (oldArmorFile.exists()) {
      if (!oldArmorFile.renameTo(
              new File(modDirectory + "armor.default.yml"))) {
        getLogger().warning("Failed to rename armor.yml to armor.default.yml!");
      }
    }

    if (oldBlocksFile.exists()) {
      if (!oldBlocksFile.renameTo(
              new File(modDirectory + "blocks.default.yml"))) {
        getLogger().warning(
            "Failed to rename blocks.yml to blocks.default.yml!");
      }
    }

    if (oldEntitiesFile.exists()) {
      if (!oldEntitiesFile.renameTo(
              new File(modDirectory + "entities.default.yml"))) {
        getLogger().warning(
            "Failed to rename entities.yml to entities.default.yml!");
      }
    }

    if (oldToolsFile.exists()) {
      if (!oldToolsFile.renameTo(
              new File(modDirectory + "tools.default.yml"))) {
  ...
  private void registerCoreSkills() {
    ...
    if (CoreSkillsConfig.getInstance().isPrimarySkillEnabled(
  ...
  private void scheduleTasks() {
    ...
    new SaveTimerTask().runTaskTimer(this, saveIntervalTicks,
                                     saveIntervalTicks);
    ...
    new BleedTimerTask().runTaskTimer(this, 1 * Misc.TICK_CONVERSION_FACTOR,
                                      1 * (Misc.TICK_CONVERSION_FACTOR / 2));

    // Old & Powerless User remover
    long purgeIntervalTicks = Config.getInstance().getPurgeInterval() * 60L *
                              60L * Misc.TICK_CONVERSION_FACTOR;

    if (purgeIntervalTicks == 0) {
      new UserPurgeTask().runTaskLaterAsynchronously(
          this,
          2 * Misc.TICK_CONVERSION_FACTOR); // Start 2 seconds after startup.
    } else if (purgeIntervalTicks > 0) {
      new UserPurgeTask().runTaskTimerAsynchronously(this, purgeIntervalTicks,
                                                     purgeIntervalTicks);
    }

    // Automatically remove old members from parties
    long kickIntervalTicks = Config.getInstance().getAutoPartyKickInterval() *
                             60L * 60L * Misc.TICK_CONVERSION_FACTOR;

    if (kickIntervalTicks == 0) {
      new PartyAutoKickTask().runTaskLater(
          this,
          2 * Misc.TICK_CONVERSION_FACTOR); // Start 2 seconds after startup.
    } else if (kickIntervalTicks > 0) {
      new PartyAutoKickTask().runTaskTimer(this, kickIntervalTicks,
                                           kickIntervalTicks);
    }

    // Update power level tag scoreboards
    new PowerLevelUpdatingTask().runTaskTimer(
        this, 2 * Misc.TICK_CONVERSION_FACTOR, 2 * Misc.TICK_CONVERSION_FACTOR);

    if (getHolidayManager().nearingAprilFirst()) {
      new CheckDateTask().runTaskTimer(this, 10L * Misc.TICK_CONVERSION_FACTOR,
                                       1L * 60L * 60L *
  ...
  private void checkModConfigs() {
    if (!Config.getInstance().getToolModsEnabled()) {
      ...
      getLogger().info(
          "To enable, set Mods.Tool_Mods_Enabled to TRUE in config.yml.");
    }
    ...
    if (!Config.getInstance().getArmorModsEnabled()) {
      ...
      getLogger().info(
          "To enable, set Mods.Armor_Mods_Enabled to TRUE in config.yml.");
    }
    ...
    if (!Config.getInstance().getBlockModsEnabled()) {
      ...
      getLogger().info(
          "To enable, set Mods.Block_Mods_Enabled to TRUE in config.yml.");
    }
    ...
    if (!Config.getInstance().getEntityModsEnabled()) {
      ...
      getLogger().info(
          "To enable, set Mods.Entity_Mods_Enabled to TRUE in config.yml.");
    }
  }
   ...
   */
  public static boolean isRetroModeEnabled() { return isRetroModeEnabled; }

  public static WorldBlacklist getWorldBlacklist() { return worldBlacklist; }

  public static PlatformManager getPlatformManager() { return platformManager; }
}
